...
1. **`attendance`** 테이블은 중복된 데이터가 쌓이는 구조이다. 중복된 데이터는 어떤 컬럼인가?
    ...
    crew_id, nickname이 중복되는 것을 확인할 수 있다.
    
2. **`attendance`** 테이블에서 중복을 제거하기 위해 **`crew`** 테이블을 만들려고 한다. 어떻게 구성해 볼 수 있을까?
    
    crew_id, nickname으로 구성 할 수 있을 것 같다.
...
1.  attendance 테이블에서 크루 정보를 추출해서 crew 테이블에 삽입하기:

```sql
INSERT INTO `crew` SELECT DISTINCT crew_id, nickname FROM attendance;
```
...
```sql
ALTER TABLE attendance ADD CONSTRAINT FOREIGN KEY(crew_id) REFERENCES crew(crew_id);
```
...
```sql
SELECT crew.nickname, attendance.attendance_date, attendance.start_time, attendance.end_time FROM crew INNER JOIN attendance ON crew.crew_id = attendance.crew_id WHERE crew.nickname = '어셔' AND attendance.attendance_date = '2025-03-06';
```
...
```sql
INSERT INTO crew(nickname) VALUES ('어서');

INSERT INTO attendance(crew_id, attendance_date, start_time, end_time) VALUES (13, '2025-03-06', '09:31', '18:01');
```
...
```sql
UPDATE attendance SET start_time = '10:00' WHERE attendance_date = '2025-03-12' AND crew_id = (SELECT crew_id FROM crew WHERE nickname = '주니');
```
...
```sql
DELETE FROM attendance WHERE attendance_date = '2025-03-12' AND crew_id = (SELECT crew_id FROM crew WHERE nickname='아론');
```
...
```sql
SELECT crew.nickname, attendance.* FROM crew INNER JOIN attendance ON crew.crew_id = attendance.crew_id;
```
...
```sql
SELECT * FROM attendance WHERE crew_id = (SELECT crew_id FROM crew WHERE nickname = '?');
```
...
```sql
SELECT nickname FROM crew WHERE crew_id = (SELECT crew_id FROM attendance WHERE attendance_date = '2025-03-05' AND end_time = (SELECT MAX(end_time) FROM attendance WHERE attendance_date = '2025-03-05'));
```
...
```sql
SELECT crew_id, COUNT(attendance_date) FROM attendance GROUP BY crew_id;
```

### **문제 14: 크루별로 등교 기록이 있는(start_time IS NOT NULL) 날짜 수 조회**

```sql
SELECT crew_id, COUNT(attendance_date) FROM attendance WHERE start_time IS NOT NULL GROUP BY crew_id;
```

### **문제 15: 날짜별로 등교한 크루 수 조회**

```sql
SELECT attendance_date, COUNT(crew_id) FROM attendance WHERE start_time IS NOT NULL GROUP BY attendance_date;
```

### **문제 16: 크루별 가장 빠른 등교 시각(MIN)과 가장 늦은 등교 시각(MAX)**

```sql
SELECT crew_id, MIN(start_time), MAX(start_time) FROM attendance GROUP BY crew_id;
```